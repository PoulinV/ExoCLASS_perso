...
int distortions_init(struct precision * ppr,
  ...
  if(pth->run_DH_with_SD == _TRUE_ && psd->loop_over_CLASS_for_DH == 0){
    ...
    psd->z_output_sd = 0; //update to default value.
...
int distortions_free(struct distortions * psd) {
  ...
  if (psd->has_distortions == _TRUE_) {
    ...
    if(psd->exact_y == _TRUE_){
        free(psd->exact_integrand_y);
    }
...
int distortions_indices(struct distortions * psd) {
  ...
  class_define_index(psd->index_type_residuals,_TRUE_,index_type,1);
...
int distortions_compute_branching_ratios(struct precision * ppr,
  ...
  if (psd->sd_branching_approx != bra_exact) {
    ...
    psd->br_table[psd->index_type_residuals][index_z] = 0;
...
int distortions_compute_heating_rate(struct precision* ppr,
  ...
  class_alloc(psd->dQrho_dz_tot,
              ...
              psd->error_message);
  if(psd->exact_y == _TRUE_){
    /** Allocate space for exact integrand function */
    /** (T_M-T_CMB)sigma_Tn_e / m_e */
    /** see eg. https://arxiv.org/pdf/2303.07366 Eq 5 */
    class_alloc(psd->exact_integrand_y,
                psd->z_size*sizeof(double*),
                psd->error_message);
  }
  ...
  for (index_z=0; index_z<psd->z_size; ++index_z){
    ...
    if(psd->z[index_z] < psd->z_output_sd){
      ...
      if(psd->exact_y == _TRUE_){
        psd->exact_integrand_y[index_z] = 0;
      }
      ...
      if(psd->exact_y == _TRUE_){
        ...
        class_call(thermodynamics_at_z(pba,
                                       pth,
                                       psd->z[index_z],  /* redshift z=1/a-1 */
                                       inter_closeby,
                                       &last_index_thermo,
                                       pvecback,
                                       pvecthermo),
         ...
         double Tgamma = pba->T_cmb * (1.+psd->z[index_z]);
         double dT     = pvecthermo[pth->index_th_Tb] - Tgamma;

         /* physical electron density */
         double ne_phys = pvecthermo[pth->index_th_xe] * pth->n_e * pow(1.+psd->z[index_z],3);

         /* dt/dz = -1/[(1+z) H]  and  (1+z)=1/a  -> dt/dz = -a/H */
         double H_in_sec = H;                                                              // [1/s]

         double dtaudz =  a / H_in_sec;

         /* dimensionless temperature ratio */
         double thetae = ne_phys * _sigma_ * _c_ *  _k_B_ * dT / (_m_e_ * _c_ * _c_);
         // printf("psd->z[index_z] %e dT %e\n",psd->z[index_z],dT,s );
         /* dy/dz */
         psd->exact_integrand_y[index_z] = 4. * thetae * dtaudz;
        // psd->exact_integrand_y[index_z] = 4*(pvecthermo[pth->index_th_Tb]-pba->T_cmb*(1+psd->z[index_z]))* _sigma_/_m_e_/_c_ * pvecthermo[pth->index_th_xe]*pth->n_e*pow(1.+psd->z[index_z],3)*a/H;
      }
...
int distortions_compute_spectral_shapes(struct precision * ppr,
  ...
  for (index_type=0; index_type<psd->type_size; ++index_type){
    if (index_type==psd->index_type_residuals) psd->sd_parameter_table[index_type]=0;
    else{
    ...
    class_call(array_trapezoidal_convolution(psd->dQrho_dz_tot,
               ...
               psd->error_message,
               psd->error_message);
      if (index_type==psd->index_type_y && psd->exact_y == _TRUE_){
        class_call(array_trapezoidal_convolution(psd->exact_integrand_y,
                                                 psd->br_table[index_type],
                                                 psd->z_size,
                                                 psd->z_weights,
                                                 &(psd->sd_parameter_table[index_type]),
  ...
  if (psd->sd_branching_approx != bra_exact || psd->sd_PCA_size == 0) {
    ...
    for (index_x=0; index_x<psd->x_size; ++index_x){
      ...
      psd->sd_shape_table[psd->index_type_residuals][index_x] = 0;         // [-]
    }
  }
  else {
  ...
  for (index_x=0;index_x<psd->x_size;++index_x){
    ...
    for (index_type=0;index_type<psd->type_size;++index_type){
      if (index_type==psd->index_type_g) {
        ...
        if (index_type!=psd->index_type_residuals) {
          // printf("index_type %d  psd->sd_parameter_table[index_type] %e psd->sd_shape_table[index_type][index_x] %e \n", index_type,  psd->sd_parameter_table[index_type],psd->sd_shape_table[index_type][index_x]);
      ...
      for (index_x=0;index_x<psd->x_size;++index_x){
                ...
                if(x > psd->DH_dist_table[3*(psd->DH_eng_size-1)]){
                  ...
                  class_call(array_interpolate_spline_transposed(psd->DH_dist_table,
	                  ...
	                  psd->sd_table[psd->index_type_residuals][index_x] = DH_distortion;
                }
                else{
                  ...
                  psd->sd_table[psd->index_type_residuals][index_x] =DH_distortion-psd->DI[index_x];
                  psd->DI[index_x] = DH_distortion;
    ...
    if(psd->include_DH_SMresidual_distortions == _TRUE_){
      ...
      for (index_x=0;index_x<psd->x_size;++index_x){
                ...
                // }
                //simple extrapolation as 0 (i.e. no distortion) outside of the range computed by DH.
                if(x > psd->DH_SMresiduals_table[3*(psd->DH_SMresiduals_eng_size-1)]){
                  DH_distortion = 0;
                }
                else if(x < psd->DH_SMresiduals_table[0]){
                  DH_distortion = 0;

                }else{


                  class_call(array_interpolate_spline_transposed(psd->DH_SMresiduals_table,
...
int distortions_output_heat_titles(struct distortions * psd,
  ...
  class_store_columntitle(titles,"exact integrand y [-]",psd->exact_y);
...
int distortions_output_heat_data(struct distortions * psd,
  ...
  for (index_z=0; index_z<psd->z_size; index_z++) {
    ...
    class_store_double(dataptr, psd->exact_integrand_y[index_z], psd->exact_y, storeidx);
  }
...
int distortions_output_sd_titles(struct distortions * psd,
  ...
  for (index_type=0;index_type<psd->type_size;++index_type){
    ...
    if (index_type==psd->index_type_residuals) {
      class_sprintf(temp_title,"SD[residuals]");
    }
    if (index_type>=psd->index_type_PCA) {
